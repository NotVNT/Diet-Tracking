...
# INPUT_DIR = "testing/graphrag_project/output"
...
#     )
# )
# description_embedding_store.connect(db_uri="db://diet-tracking-m7wx0t")
...
# : covariates are turned off by default, because they generally need prompt tuning to be valuable
...
# api_key = os.environ["HF_TOKEN"]
api_key = "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"
...
async def local_search():
    user_profile = f"""Dưới đây là thông tin của người dùng: ***người dùng có chiều cao là: 171 cm, cân nặng là 60kg, không có dị ứng, mục tiêu: tăng cân, cân nặng mục tiêu là 65 kg***.
    và tiếp theo là query của người dùng
    """

    json_type = """
        Bắt buộc trả về dữ liệu theo dạng json sau và ***không cần giải thích thêm***:
        {
            "name": string,
            "calories": float,
            "carbs": float,
...
"""
    result = await search_engine.search("quả táo có bao nhiêu calo")
    # result = await search_engine.search(user_profile + "đề xuất 5 món ăn sáng" + json_type)
    json_blocks = re.findall(r'\{.*?\}', result.response, re.DOTALL)
    foods = [json.loads(block) for block in json_blocks]
