...
void main() {
  ...
  group('ChatProvider', () {
    ...
    test('createNewChatSession sets currentSession and marks as unsaved (no save yet)', () async {
    ...
    test('sendMessage returns validation error and does not call send usecase', () async {
      final session = ChatSessionEntity.createNew(id: 's1');
      when(createNewChatSessionUseCase.execute()).thenAnswer((_) async => session);
      await provider.createNewChatSession();

      when(validateMessageUseCase.execute(any)).thenReturn(
        ValidationResult.failure('bad'),
      );
    ...
    test('sendMessage success: adds user+bot message, toggles loading, and saves session updates', () async {
      final session = ChatSessionEntity.createNew(id: 's1');
      when(createNewChatSessionUseCase.execute()).thenAnswer((_) async => session);
      await provider.createNewChatSession();
      ...
      expect(provider.messages.where((m) => !m.isUser).length, 2); // welcome + bot

      // allow async void saves to run
      await Future<void>.delayed(Duration.zero);

      // user msg triggers first save (unsaved->saved), bot msg triggers second save
      verify(chatSessionRepository.saveSession(any)).called(greaterThanOrEqualTo(2));
    });
    ...
    test('sendMessage failure: adds bot error message', () async {
      ...
      when(sendMessageUseCase.execute('hello')).thenAnswer(
        (_) async => SendMessageResult.failure('server down'),
      );
    ...
    test('sendMessage exception: adds user-friendly error message', () async {
      ...
      expect(err, 'Không thể gửi tin nhắn. Vui lòng kiểm tra kết nối và thử lại.');
      expect(provider.messages.last.isUser, false);
    ...
    test('initOrLoadRecentSession loads local current session id first', () async {
      ...
      when(chatSessionRepository.getMostRecentSessionIdFromCloud()).thenAnswer((_) async => 's2');
    ...
    test('initOrLoadRecentSession falls back to cloud id when local id is null', () async {
      final session = ChatSessionEntity.createNew(id: 's2');
      when(chatSessionRepository.getCurrentSessionId()).thenAnswer((_) async => null);
      when(chatSessionRepository.getMostRecentSessionIdFromCloud()).thenAnswer((_) async => 's2');
      when(chatSessionRepository.getSessionById('s2')).thenAnswer((_) async => session);
    ...
    test('initOrLoadRecentSession sets currentSession null when no ids found', () async {
      when(chatSessionRepository.getCurrentSessionId()).thenAnswer((_) async => null);
    ...
    test('sendFoodScanAnalysis uses prompt builder + passes extraContext to send usecase', () async {
      final session = ChatSessionEntity.createNew(id: 's1');
      when(createNewChatSessionUseCase.execute()).thenAnswer((_) async => session);
      await provider.createNewChatSession();
      ...
      when(sendMessageUseCase.execute(
        ...
        final context = inv.namedArguments[#extraContext] as Map<String, dynamic>;
        expect(context, contains('food_scan'));
        return SendMessageResult.success('ok');
      });
        when(sendMessageUseCase.execute(any)).thenAnswer(
          (_) async => SendMessageResult.success('unused'),
        );
